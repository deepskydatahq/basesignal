...
- Freeform description of the mission idea (e.g., `/plan-mission remove Convex and Clerk from the codebase`)
...
- **Vague criteria:** "Code is cleaner" is not a testing criterion. "No files import from convex/" is.
...
Input: `/plan-mission remove Convex and Clerk from the codebase`

Phase 2 explores: files importing convex, clerk config, auth flows, storage patterns, which packages depend on what.

Phase 3 presents: "142 files import from convex/, 23 use Clerk. The packages/ code has zero convex imports. Option A: remove everything. Option B: remove from packages/ only (already done). Option C: phase 1 = quarantine convex/clerk to legacy-webapp/, phase 2 = delete later."

Phase 4 defines: in scope = remove convex/clerk imports from all active code, out of scope = rebuilding the web app, acceptable breakage = web app won't work.

Phase 5 creates: `product/missions/M009-remove-legacy-dependencies.toml` with full context.
