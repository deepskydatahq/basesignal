...
import type { LlmProvider, ValueMoment } from "../types.js";
...
export interface ActivationMapResult {
...
export function parseActivationMapResponse(responseText: string): ActivationMapResult {
...
export async function generateActivationMap(
  ...
  primaryActivation: number,
  llm: LlmProvider,
): Promise<ActivationMapResult> {
  const userPrompt = buildActivationMapUserPrompt(levels, valueMoments, primaryActivation);
