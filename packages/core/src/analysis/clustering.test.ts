...
function makeCandidate(
  overrides: Partial<ValidatedCandidate> & { id: string; lens: ExperientialLensType; name: string; description: string }
): ValidatedCandidate {
  return {
    confidence: 0.8,
...
describe("integration: multi-lens clustering", () => {
  ...
  it("handles a realistic set of 20 candidates without duplicate IDs in output", () => {
    const lenses: ExperientialLensType[] = ["jtbd", "outcomes", "pains", "gains", "workflows"];
    const topics = [
