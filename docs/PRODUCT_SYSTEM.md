...
2. `/product-epic` → Break into epic with child issues
3. Dev workflow: `/brainstorm` → `/plan-issue` → `/pick-issue`
4. `/product-iteration` → Analyze what we learned
5. Update HYPOTHESES.md with results
6. Repeat
...
└── docs/
    └── plans/       # Design documents from /new-feature
```
...
| Break hypothesis into tasks | `/product-epic` | GitHub issues |
